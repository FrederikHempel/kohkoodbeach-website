...
*(Both open items above were closed when the beat was deployed — see the
deployment entry further down.)*
...
### "Where you'll stay" deployed to the homepage (8 Sep 2026)

Frederik: "Let's deploy it to the website." The beat lives on `index.html`
now, between the editorial intro and the marquee, and **it replaced two
sections, not one**: the pinned beach photograph (`img4p29`, "A private beach
that stays quiet…") *and* the sage room slider. The aerial contains the beach,
and three pinned photo stages in a row was more scrolling than the fourth beat
could earn. The beach band is a `git show` away if it is wanted back.

**Where things live now:**

| what | where |
|---|---|
| markup | generated by `scratchpad/build_plot.py`, spliced between the `WHERE YOU'LL STAY` markers in `index.html` — re-run after editing it or the cards |
| house names, one-liners, Size/Beds/Sleeps, from-prices | **read from `accommodation.html`'s `.cat` cards** by the generator, so the two pages cannot disagree; only the photograph per house is chosen in the script |
| styles | `style.css`, "Where you'll stay — the plot from above" at the end of the file |
| behaviour | `initPlot()` in `script.js`, called after `initRoute()` |
| the hand-lettered names | a second `@import` in `style.css`: Fraunces italic with its `SOFT`/`WONK` axes — no handwriting font |

**"Learn more" opens the house it names.** `initRoomDetail()` now reads the
hash on load and calls `openFor()` when it matches a card — measured on
`accommodation.html#bali-deluxe`: `aria-expanded` true, label "See less", the
panel visible, the card at 138px (under the nav, not behind it). That also
fixes the old slider's links, which always landed on a closed card.

**The sage slider is retired:** its markup, `initRoomSlider()` (`initRoomSlider2`
on the rooms page is a different thing and stays), and every `.rooms__*` rule.
`staging-plot.html` is deleted; `scratchpad/staging/` is fully ignored again.

⚠️ **Removing `.rooms` selectors by text took the rooms page's `.rooms-top` and
`.rooms-top__lead` with it** — a filter for `.rooms` matched `.rooms-top`, dropped
the opener line of a multi-line rule, and left its body orphaned, which the
brace count caught (731 vs 732). Both restored from git. A class prefix is not
a namespace; when retiring a block, match the exact selectors, and let the
brace count and a depth walk run before anything visual.

Verified in headless Chrome (rAF runs there; the pane cannot do this): the
slider and beach band gone, the pool band kept, the state machine walking
opening beat → Bali House → Bali Deluxe → Thai Twin with the restaurant
falling to its faded landmark state; no page errors; guard green on 12 pages;
detector's only findings the five accepted `transition: height` ones.

### `--nav-h` was stale on every page with a hero (8 Sep 2026)

Found by the deployment above: the pinned plot sat 40px below the solid nav
with the section's charcoal ground showing in the gap. Measured at 1440: the
nav is **114px** over the hero and **74px** once solid, and `--nav-h` stayed at
`114px` after the switch.

⚠️ **`publishNavHeight()`'s ResizeObserver watched the content box** — the
default — and `.nav--solid` only changes the nav's *padding*, so the content
box never moved and the observer never fired. It observes `{ box: 'border-box' }`
now, and `--nav-h` reads `74px` when solid. Every consumer benefits: the plot
